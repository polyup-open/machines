...
/// ported to Typescript and adapted for Polyup puzzles by Donald Goldin
...
export class Diff {
  ...
  public static DiffText(
    ...
    var h = new Map<string, number>();

    // The A-Version of the data (original data) to be compared.
    var DataA: DiffData = new DiffData(
      this.DiffCodes(TextA, h, trimSpace, ignoreSpace, ignoreCase)
    );

    // The B-Version of the data (modified data) to be compared.
    var DataB: DiffData = new DiffData(
      this.DiffCodes(TextB, h, trimSpace, ignoreSpace, ignoreCase)
    );

    h = null; // free up hashtable memory (maybe)

    var MAX: number = DataA.Length + DataB.Length + 1;
    /// vector for the (0,0) to (x,y) search
    var DownVector: number[] = [];
  ...
  static Optimize(Data: DiffData): void {
    var StartPos: number;
    var EndPos: number;
  ...
  /// <returns>Returns a array of Items that describe the differences.</returns>
  static DiffInt(ArrayA: number[], ArrayB: number[]): Item[] {
    // The A-Version of the data (original data) to be compared.
  ...
  static DiffStringArray(ArrayA: string[], ArrayB: string[]): Item[] {
    // The A-Version of the data (original data) to be compared.
    var DataA = new DiffData(ArrayA);
  ...
  /// </summary>
  /// dynamic array
  static CreateDiffs(DataA: DiffData, DataB: DiffData): Item[] {
    var a: Item[] = [];
    var aItem: Item;

    var StartA: number, StartB: number;
    var LineA: number, LineB: number;
...
class DiffData {
  ...
  constructor(initData: number[] | string[]) {
    this.data = initData;
